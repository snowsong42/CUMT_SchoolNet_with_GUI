...
class QuotesManager:
    ...
    def load_Loji_quotes(self):
        ...
        quotes_paths = [
            ...
            os.path.join(base_dir, 'resource', 'Loji_quotes.txt'),  # 主程序目录下的resources文件夹
        ...
        for quote_path in quotes_paths:
            try:
                if os.path.exists(quote_path):
                    ...
                    self.thread_handler.log_message(f"成功加载 {len(self.Loji_quotes)} 条留言，从路径: {quote_path}")
                    return
            except Exception as e:
                self.thread_handler.log_message(f"尝试加载留言文件 {quote_path} 失败: {e}")
                continue
